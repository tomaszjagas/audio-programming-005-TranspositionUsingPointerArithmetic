/********************************************************
Description: Program calculates the result of transposing
a note by any number of semitones (positive or negative)
*********************************************************/

#include <stdio.h>
#include <string.h>

// function that keep modulo 12
int mod12(int note) {
	while (note < 0) {
		note += 12;
	}
...
int main() {
	...
	// point p1 to the beginning of array and p2 to its end
	p1 = table;
	p2 = table + 11;

	// find the base note position, incrementing pointer until it's found
	while (strcmp(*p1, note)) {
		p1++;
		if (p1 > p2) { // if end is passed
